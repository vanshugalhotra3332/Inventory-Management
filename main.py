...
class StoreBook:
    def __init__(self):
        ...
        def exit_system(screen):
            if screen == root:
                ans = tkinter.messagebox.askyesno('Quit!', 'Confirm If You Want To Exit!')
                if ans is True:
                    end_func()
            else:
                screen.destroy()
        ...
        def delete_product_finally(_event=None):
            ...
            if del_name.lower() in all_products:
                permission = tkinter.messagebox.askyesno('Permission Required!', f"Confirm to delete product'{del_name}'")
                if permission is True:
                    ...
                    if rd_qty == 0: # if quantity for product is 0 then and only then it can be deleted
                        ...
                        cursor.execute(com)
                        connection.commit()
                        tkinter.messagebox.showinfo('Success!', f"Product '{del_name}' delete successfully!")
                        delete_window.destroy()

                    else:
                        tkinter.messagebox.showerror('Error!', f"Quantity for Product '{del_name}' is not 0 so Product can't be deleted")

            else:
                tkinter.messagebox.showerror('Error!', f"'{del_name}' product name doesn't exists")
        ...
        def recently_added():
            ...
            def search_(sort_by):
                ...
                if searched != 0:
                    try:
                        reca_treeview.delete(*reca_treeview.get_children())
                        command_ = f"SELECT product_name FROM recently_added WHERE {sort_by} LIKE '{parameter}'"
                        ...
                        for names in fetch_list:
                            for items in total_data:
                                if names == items[1]:
                                    reca_treeview.insert('', 'end', values=items)
            ...
            menu_bar = ttk.Menubutton(reca_window, text='Sort By', cursor='mouse')
            menu_bar.grid(row=1, column=2)
            menu_ = Menu(menu_bar, tearoff=0)
            menu_bar["menu"] = menu_
            menu_.add_command(label='Brand Name', command=lambda sort_by="brand_name": search_(sort_by))
            menu_.add_command(label='Part Number', command=lambda sort_by="part_number": search_(sort_by))
            menu_.add_command(label='MRP', command=lambda sort_by="mrp": search_(sort_by))
            ...
            reca_treeview = ttk.Treeview(reca_window, column=addition_columns, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(reca_window, orient='vertical', command=reca_treeview.yview)
            ...
            # search bar
            sb_entry = Entry(reca_window, textvariable=addition_search, width=110, font=('arial', 15))
            sb_entry.grid(row=1, column=0)

            sb_entry.bind('<KeyRelease>', lambda event, sort_by='product_name': search_(sort_by) )
        ...
        def recently_deleted():
            ...
            def search_(sort_by):
                ...
                if searched != 0:
                    try:
                        rec_treeview.delete(*rec_treeview.get_children())
                        command_ = f"SELECT product_name FROM recently_deleted WHERE {sort_by} LIKE '{parameter}'"
                        cursor.execute(command_)
                        fetch = cursor.fetchall()
                        fetch_list = list(itertools.chain(*fetch))
                        for names in fetch_list:
                            for items in total_data:
                                if names == items[1]:
                                    rec_treeview.insert('', 'end', values=items)
            ...
            """menu button________________________"""
            menu_bar = ttk.Menubutton(rec_window, text='Sort By', cursor='mouse')
            menu_bar.grid(row=1, column=2)
            menu_ = Menu(menu_bar, tearoff=0)
            menu_bar["menu"] = menu_
            menu_.add_command(label='Brand Name', command=lambda sort_by="brand_name": search_(sort_by))
            ...
            # treeview
            global rec_treeview
            rec_treeview = ttk.Treeview(rec_window, column=deletion_columns, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(rec_window, orient='vertical', command=rec_treeview.yview)
            ...
            # search bar
            sb_entry = Entry(rec_window, textvariable=deletion_search, width=110, font=('arial', 15))
            sb_entry.grid(row=1, column=0)
            ...
            # adding data to treeview fetching from table
            global total_data
            total_data = func_provider.custom_fetching(table='recently_deleted')
            for treeview_data in total_data:
        ...
        def updation_transactions():
                        ...
                        for names in fetch_list:
                            ...
                            while ts < len(data_needed):
                                if names.lower() == data_needed[ts][1].lower() or names.lower() in data_needed[ts][1].lower():
                                    tran_treeview.insert('', 'end', values=data_needed[ts], tags=('oddrow'))
                                    tran_treeview.insert('', 'end', values=data_needed[ts+1], tags=('evenrow'))
                                ts+=2
                            #
            ...
            """menu button________________________"""
            menu_bar = ttk.Menubutton(tran_window, text='Sort By', cursor='mouse')
            menu_bar.grid(row=1, column=2)
            menu_ = Menu(menu_bar, tearoff=0)
            menu_bar["menu"] = menu_
            menu_.add_command(label='Brand Name', command=lambda sort_by="brand_name": _search_(sort_by))
            menu_.add_command(label='Part Number', command=lambda sort_by="part_number": _search_(sort_by))
            menu_.add_command(label='MRP', command=lambda sort_by="mrp": _search_(sort_by))
            menu_.add_command(label='Box Number', command=lambda sort_by="box_no": _search_(sort_by))
            menu_.add_command(label='Description', command=lambda sort_by="description": _search_(sort_by))
            menu_.add_command(label='date', command=lambda sort_by="date": _search_(sort_by))

            # treeview
            global tran_treeview
            tran_treeview = ttk.Treeview(tran_window, column=updation_columns, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(tran_window, orient='vertical', command=tran_treeview.yview)
            ...
            # search bar
            sb_entry = Entry(tran_window, textvariable=updation_search, width=110, font=('arial', 15))
            sb_entry.grid(row=1, column=0)
            ...
            search_btn = Button(tran_window, text="Search", bd=1, pady=1, padx=1,
                                ...
                                bg='snow')
            search_btn.grid(row=1, column=1)
            sb_entry.bind('<KeyRelease>', lambda event, sort_by='product_name': _search_(sort_by) )
            ...
            while td < len(data_needed):
                try:
                    tran_treeview.insert('', 'end',data_needed[td], tags=('oddrow'))
                    tran_treeview.insert('', 'end',data_needed[td+1], tags=('evenrow'))
        ...
        def stock_in():
            ...
            def search_(sort_by):
                ...
                if searched != 0:
                    try:
                        si_treeview.delete(*si_treeview.get_children())
                        command_ = f"SELECT product_name FROM stock_in WHERE {sort_by} LIKE '{parameter}'"
                        cursor.execute(command_)
                        fetch = cursor.fetchall()
                        fetch_list = list(itertools.chain(*fetch))
                        for names in fetch_list:
                            for items in total_data:
                                if names == items[1]:
                                    si_treeview.insert('', 'end', values=items)
            ...
            """menu button________________________"""
            menu_bar = ttk.Menubutton(si_window, text='Sort By', cursor='mouse')
            menu_bar.grid(row=1, column=2)
            menu_ = Menu(menu_bar, tearoff=0)
            menu_bar["menu"] = menu_
            menu_.add_command(label='Brand Name', command=lambda sort_by="brand_name": search_(sort_by))
            ...
            # treeview
            global si_treeview
            si_treeview = ttk.Treeview(si_window, column=addition_columns, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(si_window, orient='vertical', command=si_treeview.yview)
            ...
            # search bar
            sb_entry = Entry(si_window, textvariable=si_search, width=110, font=('arial', 15))
            sb_entry.grid(row=1, column=0)
        ...
        def stock_out():
            ...
            def search_(sort_by):
                ...
                if searched != 0:
                    try:
                        so_treeview.delete(*so_treeview.get_children())
                        command_ = f"SELECT product_name FROM stock_out WHERE {sort_by} LIKE '{parameter}'"
                        cursor.execute(command_)
                        fetch = cursor.fetchall()
                        fetch_list = list(itertools.chain(*fetch))
                        for names in fetch_list:
                            for items in total_data:
                                if names == items[1]:
                                    so_treeview.insert('', 'end', values=items)
            ...
            """menu button________________________"""
            menu_bar = ttk.Menubutton(so_window, text='Sort By', cursor='mouse')
            menu_bar.grid(row=1, column=2)
            menu_ = Menu(menu_bar, tearoff=0)
            menu_bar["menu"] = menu_
            menu_.add_command(label='Brand Name', command=lambda sort_by="brand_name": search_(sort_by))
            ...
            # treeview
            global so_treeview
            so_treeview = ttk.Treeview(so_window, column=addition_columns, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(so_window, orient='vertical', command=so_treeview.yview)
            ...
            # search bar
            sb_entry = Entry(so_window, textvariable=so_search, width=110, font=('arial', 15))
            sb_entry.grid(row=1, column=0)
        ...
        def delete_product(_event=None):
            ...
            lbl_name = Label(delete_window, text="Delete Product Name", font=('arial', 30, 'bold'),padx=2, pady=2, bd=2)
            lbl_name.grid(row=0, column=0)
        ...
        def register_brand_name(_event=None):
            ...
            lbl_name = Label(brand_window, text="Register Brand Name", font=('arial', 30, 'bold'),padx=2, pady=2, bd=2)
            lbl_name.grid(row=0, column=0)
        ...
        def update_brand_name(_event=None):
            ...
            lbl_name = Label(brand_window_, text="Update Brand Name", font=('arial', 30, 'bold'),padx=2, pady=2, bd=2)
            lbl_name.grid(row=0, column=0)
            gui_func_provider.custom_entry_labels(brand_window_, text=' Old Brand Name:', r=1, var=old_brand_name)
            gui_func_provider.custom_entry_labels(brand_window_, text=' New Brand Name:', r=2, var=new_brand_name)
        ...
        def update_mrp_finally():
            ...
            db_part_numbers = func_provider.get_list_from_database(table='stock', field='part_number')
            ...
            if file_path != '':
                ...
                try:
                    ...
                    for part_no in db_part:
                        mrp_record = record_df[record_df[part_column].str.lower()== part_no]
                    ...
                    part_mrp_dict = {db_part[i]: mrp_list_needed[i] for i in range(len(db_part))}
                    print(part_mrp_dict)
                    ...
                    connection.commit()
                    gui_func_provider.focus_on(mrp_window, state=False)
                    tkinter.messagebox.showinfo('Success!', f'Total {len(part_mrp_dict)} Records Updated!')
                    mrp_window.destroy()

                except KeyError:
                    tkinter.messagebox.showerror('Error', f"Column Name for Part Number '{part_column}' or for MRP '{mrp_column}' is wrong!")
        ...
        def update_mrp(_event=None): 
            def pdf_picker():
                ...
                files = [ ('Microsoft Excel File','.xlsx'), ('Microsoft Excel File','.xls')]
                open_path_ = filedialog.askopenfilename(filetypes=files, initialdir=cur_wd)
            ...
            lbl_name = Label(mrp_window, text="Update MRP", font=('arial', 30, 'bold'),padx=2, pady=2, bd=2)
            lbl_name.grid(row=0, column=0)

            gui_func_provider.custom_entry_labels(mrp_window, text='Part Number Column Name:', var=part_ind, r=2)
            gui_func_provider.custom_entry_labels(mrp_window, text='MRP Column Name:', var=mrp_ind, r=3)
            gui_func_provider.custom_entry_labels(mrp_window, text='Excel File:', var=pdf_ind, r=4, state='disabled')
        ...
        def total_stock(_event=None):
            stock_list = func_provider.get_list_from_database(table='stock', field='mrp*quantity')
            sum_stock = sum(stock_list)
            tkinter.messagebox.showinfo('Total Stock!', f'Total Stock is Rs. {sum_stock}')
        ...
        def export_xlsx(save_dir):
            ...
            slice_product_name = slice(0, len(record_list), 12)  # slicing all product names from list
            slice_tractor = slice(1, len(record_list), 12)  # slicing all tractor from list
            slice_brand_name = slice(2, len(record_list), 12)  # slicing all brand names from list
            slice_part_no = slice(3, len(record_list), 12)  # slicing all part numbers from list
            slice_code = slice(4, len(record_list), 12)  # slicing all code  from list
            slice_mrp = slice(5, len(record_list), 12)  # slicing all mrp from list
            slice_box_no = slice(6, len(record_list), 12)  # slicing all box_no from list
            slice_desc = slice(7, len(record_list), 12)  # slicing all description from list
            slice_qty = slice(8, len(record_list), 12)  # slicing all quantity from list
            slice_warning_qty = slice(9, len(record_list), 12)
            slice_date = slice(10, len(record_list), 12)  # slicing all date from list
            slice_img = slice(11, len(record_list), 12)  # slicing all img from list
            ...
            if len(save_dir) != 0:
                ...
                df.to_excel(writer, sheet_name='Records', index=False, columns=columns)
        ...
        def update_finally(_event=None):
            ...
            try:
                ...
                tkinter.messagebox.showerror('Error!', 'MRP , Quantity and Warning Quantity must be number')
                gui_func_provider.focus_on(up_screen)

            if tractor_up == '':
                tkinter.messagebox.showerror('Error!', "Tractor Name Can't be empty!")
                gui_func_provider.focus_on(up_screen)

            elif quantity_up == '':
                tkinter.messagebox.showerror('Error!', "Quantity Can't be empty")
                gui_func_provider.focus_on(up_screen)
                ...
                tkinter.messagebox.showinfo('Success', f"Record '{product_name_up}' updated successfully!")
        ...
        def update(_event=None):
            ...
            if len(values_data) != 0:
                try:
                    ...
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Product Name:*', r=1, var=up_product_name, state='disabled')
                    """____________________________tractor_name____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Tractor Name:', r=2, var=up_tractor_name)

                    """____________________________brand_name____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Brand Name:*', r=3, var=up_brand_name, state='disabled')
                    """____________________________part number____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Part Number:', r=4, var=up_part_no, state='disabled')

                    """____________________________code____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Code:', r=5, var=up_code)

                    """____________________________mrp____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='MRP:*', r=6, var=up_mrp)

                    """____________________________box no____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Box Number:', r=7, var=up_box_no)
                    """____________________________description____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Description:', r=8, var=up_description)

                    """____________________________Quantity____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Quantity:*', r=9, var=up_quantity)

                    """____________________________Warning Quantity____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Warning Quantity:*', r=10, var=up_warning_qty)

                    """____________________________date____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Date:',r=11,var=up_date,state='disabled')
                    """_________________________________Image_____________________________________"""
                    gui_func_provider.custom_entry_labels(up_ent_frame, text='Image:',r=12,var=up_img,state='disabled')
        ...
        def inventory(_event=None):
            ...
            def search_(sort_by):
                ...
                if searched != 0:
                    try:
                        ...
                        command_ = f"SELECT * FROM stock WHERE {sort_by} LIKE '{parameter}'"
                        cursor.execute(command_)
                        fetch = cursor.fetchall()
                        for data in fetch:
                        ...
                        pass
            """menu button________________________"""
            menu_bar = ttk.Menubutton(inv_screen, text='Sort By', cursor='mouse')
            ...
            # search bar
            sb_entry = Entry(inv_screen, textvariable=inv_search, width=110, font=('arial', 15))
            ...
            records_treeview = ttk.Treeview(inv_screen, column=every_damn_field, show='headings', height=30, cursor='hand1')
            scroll_bar = ttk.Scrollbar(inv_screen, orient='vertical', command=records_treeview.yview)
            records_treeview.configure(yscroll=scroll_bar.set)
            scroll_bar.grid(row=3, column=2, sticky='ns')
            for a in every_damn_field:
            ...
            #fetching the damn data from database
            damn_data = func_provider.get_list_from_database('stock', field='product_name')
            for inv_data in damn_data:
                select_command = f'SELECT product_name,tractor,brand_name,part_number,code,mrp,box_no,'\
                f'description,quantity,warning_qty from stock WHERE product_name = "{inv_data}"'
                cursor.execute(select_command)
                data_tuple = cursor.fetchall()
                damn_data_list = list(itertools.chain(*data_tuple))
                records_treeview.insert('', 'end', values=damn_data_list)
            connection.commit()

            records_treeview.bind('<Control-u>', update)   # for updating values
        ...
        def add_finally(_event= None):
            ...
            try:
                ...
                tkinter.messagebox.showerror('Error!', 'MRP , Quantity and Warning Quantity must be a number')

            if product_name == '':
                tkinter.messagebox.showerror('Error!', 'Product Name is necessary!')

            elif brand_name == '':
                tkinter.messagebox.showerror('Error!', 'Brand Name is necessary!')

            elif tractor_name == '':
                tkinter.messagebox.showerror('Error!', 'Tractor Name is necessary!')

            elif quantity == '':
                tkinter.messagebox.showerror('Error!', 'Quantity is necessary!')
            ...
            #     add_part_no.set('')
                ...
                try:
                    all_brands = func_provider.get_list_from_database('brands', 'name')
                    ...
                    if brand_name.lower() in all_brands:
                        ...
                        tkinter.messagebox.showinfo('Success', f'Product "{product_name}" added Successfully!')
                        ...
                        tkinter.messagebox.showerror('Error!', f'"{brand_name}" Brand Name is not registered!')
                        register_brand_name()
                        brand_name_.set(brand_name)
                        gui_func_provider.focus_on('add_screen')

                except mysql.errors.IntegrityError:
                    tkinter.messagebox.showerror('Error', f'Product Name "{product_name}" or Part Number "{part_number}"already exists!')
        ...
        def add_inventory(_event=None):
            ...
            if len(brand_list) != 0:
                ...
                def suggest_trac(_event=None):
                    ...
                    all_brands = func_provider.get_list_from_database('stock', 'DISTINCT(tractor)')
                    for i in  range(len(all_brands)):
                        all_brands[i] = all_brands[i].lower()
                ...
                menu_button = ttk.Menubutton(add_ent_frame, text='Brand Names:', cursor='mouse')
                menu_button.grid(row=3, column=2)

                menu_ = Menu(menu_button, tearoff=0)
                menu_button['menu'] = menu_

                brands_data = func_provider.get_list_from_database('brands', 'name')
                for brands in brands_data:
                    menu_.add_command(label=f'{brands}', command = lambda br_name=brands: store_br(br_name))

                """____________________________part number____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Part Number:', r=4, var=add_part_no)

                """____________________________code____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Code:', r=5, var=add_code)

                """____________________________mrp____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='MRP:*', r=6, var=add_mrp)

                """____________________________box no____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Box Number:', r=7, var=add_box_no)
                """____________________________description____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Description:', r=8, var=add_description)

                """____________________________Quantity____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Quantity:*', r=9, var=add_quantity)

                """____________________________Warning Quantity____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Warning Quantity:*', r=10, var=add_warning_qty)

                """____________________________date____________________________________"""
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Date:',r=11,var=add_date,state='disabled')
                ...
                gui_func_provider.custom_entry_labels(add_ent_frame, text='Image:',r=12,var=add_img,state='disabled')
                ...
                try:
                    ...
                    tkinter.messagebox.showwarning('Warning!',f"Icon is deleted so program will use '{img_text}' instead of icon")
                    gui_func_provider.focus_on(add_screen)
                ...
                tkinter.messagebox.showerror('Error!', 'Register a Brand Name first!     (Ctrl+r)')
                register_brand_name()
        ...
        global treeview
        treeview = ttk.Treeview(side_frame, column=labels, show='headings', height=90, cursor='hand1')
        scroll_bar_ = ttk.Scrollbar(side_frame, orient='vertical', command = treeview.yview)
        ...
        tool_sub.add_command(label='Update Brand (Ctrl-Alt-r)', command=update_brand_name) # t2
        tool_sub.add_command(label='Total Stock (Ctrl-t)', command=total_stock) # t3
        tool_sub.add_command(label='Update MRP (Ctrl-m)', command=update_mrp) # t4
        tool_sub.add_command(label='Delete Product (Ctrl-d)', command=delete_product) # t5
